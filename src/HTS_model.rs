...
mod tests {
    ...
    fn get_parameter() {
        ...
        let len = jsyn.get_vector_length(1) * jsyn.get_window_size(1);
        let mut hts_mean = vec![0.; len];
        let mut hts_vari = vec![0.; len];
        let mut hts_msd = 0.;
        unsafe {
        ...
        if let Some(msd) = jsyn_param.msd {
            assert_eq!(hts_msd, msd as f64);
        }
    }
    ...
    fn get_gv() {
        ...
        let len = jsyn.get_vector_length(1);
        let mut hts_mean = vec![0.; len];
        let mut hts_vari = vec![0.; len];
        unsafe {
            HTS_ModelSet_get_gv(
    ...
    fn window() {
        let (mut hts, jsyn) = load_models();
        assert_eq!(
            unsafe { HTS_ModelSet_get_window_size(&mut hts, 0) },
            jsyn.get_window_size(0) as u64
        );
        assert_eq!(
            unsafe { HTS_ModelSet_get_window_left_width(&mut hts, 0, 1) },
            jsyn.get_window_left_width(0, 1)
        );
        assert_eq!(
            unsafe { HTS_ModelSet_get_window_right_width(&mut hts, 0, 1) },
            jsyn.get_window_right_width(0, 1)
        );
        assert_eq!(
            unsafe { HTS_ModelSet_get_window_coefficient(&mut hts, 0, 1, 0) },
            jsyn.get_window_coefficient(0, 1, 0) as f64
        );
        assert_eq!(
            unsafe { HTS_ModelSet_get_window_coefficient(&mut hts, 0, 1, 1) },
            jsyn.get_window_coefficient(0, 1, 1) as f64
        );
        assert_eq!(
            unsafe { HTS_ModelSet_get_window_max_width(&mut hts, 0) },
            jsyn.get_window_max_width(0) as u64
        );
    }
}
